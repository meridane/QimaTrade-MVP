...
export default function NewProductRequestPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!data.request?.id) {
        setError("The request was created but no request ID was returned.");
        return;
      }
  ...
  async function publishAndContinue() {
    if (!createdId || publishing) return;
    ...
    try {
      ...
      // The publish API wraps the action result inside result.result.
      // Keep fallbacks for older deployments/API responses.
      const demandId =
        data.result?.result?.demandId ??
        data.result?.demandId ??
        data.demandId ??
        null;
      ...
      if (!demandId) {
        setError(
          "The request was published, but no demand ID was returned.",
        );
        return;
      }

      router.push(
        `/demands/qualification?id=${encodeURIComponent(demandId)}`,
      );
    } catch (caughtError) {
      setError(
        caughtError instanceof Error
      ...
      <main className="min-h-screen bg-slate-50 px-5 py-10">
        <div className="mx-auto max-w-xl rounded-3xl border border-red-200 bg-white p-8 shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Start from the Decision Tree and select a Product Master first.
          </p>
          <Link
            href="/decision-tree"
            className="mt-6 inline-flex rounded-xl bg-orange-500 px-5 py-3 text-sm font-bold text-white"
          >
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <div className="mt-6 rounded-3xl border border-slate-200 bg-white p-8 shadow-sm sm:p-10">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-500">
            The request is already linked to the Product Master selected during
            classification.
          </p>
            ...
            <section className="mt-8 rounded-2xl border border-emerald-200 bg-emerald-50 p-6">
              ...
              <div className="mt-5 flex flex-col gap-3 sm:flex-row">
                <button
                  ...
                  disabled={publishing}
                  className="inline-flex justify-center rounded-xl bg-orange-500 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-orange-500/20 transition hover:bg-orange-600 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  {publishing
                    ? "Publishing…"
                    : "Publish & continue to qualification →"}
                </button>

                <Link
                  href="/decision-tree"
                  className="inline-flex justify-center rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-700"
                >
            ...
            <form onSubmit={submit} className="mt-8 space-y-5">
              <label className="block">
                ...
                <input
                  ...
                  required
                  maxLength={180}
                  placeholder="e.g. 20 CNC turning centers"
              ...
              <label className="block">
                ...
                <textarea
                  ...
                  rows={5}
                  maxLength={4000}
                  placeholder="Specs, preferred brand, condition, delivery notes..."
                  className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-orange-400"
                />
              </label>
              ...
              >
                {saving ? "Creating request…" : "Create product request"}
              </button>
            </form>
          )}
        </div>
