...
export default async function ProjectShipmentsPage({ params }: Props) {
  ...
  return <main className="min-h-screen bg-slate-50 px-5 py-8"><div className="mx-auto max-w-6xl">
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-3">
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-xl font-black">Create shipment</h2><p className="mt-1 text-sm text-slate-500">QimaTrade suit la livraison sans exécuter le transport.</p><form action={createShipment.bind(null, id)} className="mt-5 space-y-4">
        ...
        <SubmitButton loadingText="Creating shipment..." className="w-full rounded-xl bg-orange-500 px-4 py-3 text-sm font-black text-white hover:bg-orange-600 disabled:cursor-wait disabled:opacity-70">Create shipment</SubmitButton>
      </form></section>
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm lg:col-span-2"><div className="flex items-center justify-between"><div><h2 className="text-xl font-black">Delivery workflow</h2><p className="text-sm text-slate-500">pending → pickup_scheduled → picked_up → in_transit → delivered</p></div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold">{shipments?.length ?? 0}</span></div>
        ...
        <div className="mt-5 space-y-4">{(shipments ?? []).map((s) => <div key={s.id} className="rounded-2xl border border-slate-100 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-black">{s.shipment_id}</p><p className="mt-1 text-sm font-bold">{s.pickup_location} → {s.destination}</p><p className="mt-1 text-xs text-slate-400">Order {orders?.find((o) => o.id === s.order_id)?.order_id ?? s.order_id}{s.transporter_actor_id ? ` · ${actorMap.get(s.transporter_actor_id) ?? "Transporter"}` : " · External transporter"}</p>{s.tracking_reference && <p className="mt-1 text-xs text-slate-500">Tracking: {s.tracking_reference}</p>}</div><span className="rounded-full bg-orange-50 px-3 py-1 text-xs font-black text-orange-700">{s.status}</span></div>{s.notes && <p className="mt-3 rounded-xl bg-slate-50 p-3 text-sm text-slate-600">{s.notes}</p>}<div className="mt-4 flex flex-wrap gap-2 border-t border-slate-100 pt-4">{statuses.map((status) => <form key={status} action={updateShipmentStatus.bind(null, id, s.id, status)}><SubmitButton disabled={s.status === status} loadingText="Updating..." className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold disabled:cursor-not-allowed disabled:opacity-30 hover:border-orange-300 hover:text-orange-700">{status}</SubmitButton></form>)}</div></div>)}{!shipments?.length && !shipmentsError && <div className="rounded-2xl border border-dashed border-slate-300 p-10 text-center"><p className="font-bold">No shipments yet</p><p className="mt-1 text-sm text-slate-500">Create the first shipment above.</p></div>}</div>
      </section>
    </div>
  </div></main>;
}
