...
import { assignTask, updateTaskStatus } from "./actions";
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-3">
            ...
            <div className="mt-4 space-y-3">
              {(participants ?? []).map((p) => (
                <div key={p.id} className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                  <p className="text-sm font-black">{actorMap.get(p.actor_id)?.name ?? "Participant"}</p>
                  <p className="mt-1 text-xs font-bold text-orange-600">{p.role}</p>
                  <p className="mt-1 break-all text-[11px] text-slate-400">Actor: {p.actor_id}</p>
                </div>
              ))}
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm lg:col-span-2">
            ...
            <div className="mt-4 space-y-4">
              {(tasks ?? []).map((t) => {
                const assignedName = taskActorMap.get(t.owner_actor_id)?.name;
                return (
                  <div key={t.id} className="rounded-2xl border border-slate-100 p-4">
                    <div className="flex justify-between gap-3">
                      <div><p className="font-black">{t.title}</p><p className="mt-1 text-[11px] text-slate-400">{t.task_id}</p></div>
                      <span className="rounded-full bg-orange-50 px-3 py-1 text-xs font-bold text-orange-700">{t.status}</span>
                    </div>
                    <p className="mt-2 text-xs text-slate-500">Priority: <strong>{t.priority}</strong> · Assigned: <strong>{assignedName ?? "Unassigned"}</strong></p>

                    <div className="mt-4 border-t border-slate-100 pt-4">
                      <p className="text-xs font-black uppercase tracking-wide text-slate-400">Assign task</p>
                      <div className="mt-2 flex flex-wrap gap-2">
                        {(participants ?? []).map((p) => {
                          const name = actorMap.get(p.actor_id)?.name ?? "Participant";
                          return (
                            <form key={p.actor_id} action={assignTask.bind(null, id, t.id, p.actor_id)}>
                              <button type="submit" className="rounded-xl border border-orange-200 bg-orange-50 px-3 py-2 text-xs font-bold text-orange-700 hover:bg-orange-100">Assign to {name}</button>
                            </form>
                          );
                        })}
                      </div>
                    ...
                    <div className="mt-4">
                      <p className="text-xs font-black uppercase tracking-wide text-slate-400">Change status</p>
                      <div className="mt-2 flex flex-wrap gap-2">
                        {statuses.map((status) => (
                          <form key={status} action={updateTaskStatus.bind(null, id, t.id, status)}>
                            <button type="submit" disabled={t.status === status} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700 disabled:cursor-not-allowed disabled:opacity-40 hover:border-orange-300 hover:text-orange-700">{status}</button>
                          </form>
                        ))}
                      </div>
                    </div>
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm lg:col-span-3">
            <div className="flex items-center justify-between"><div><h2 className="text-xl font-black">Timeline</h2><p className="text-sm text-slate-500">Every assignment and status change is recorded here.</p></div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold">{events?.length ?? 0}</span></div>
            <div className="mt-4 space-y-3">
