...
export default async function DemandMatchPage({ searchParams }: { searchParams: Promise<{ id?: string }> }) {
  ...
  const { data: offers } = productMasterId
    ? await supabase
        .from("offers")
        .select("id, demand_id, product_master_id, name, quantity, price, currency, market, geography, conditions")
  ...
  const evaluated = ((offers ?? []) as Offer[]).map((offer) => {
    ...
    return { offer, quantityOk, budgetOk, currencyOk, marketOk, matched: checks.filter(Boolean).length };
      ...
      <header className="rounded-3xl border border-slate-200 bg-white p-6 shadow-soft sm:p-8">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Only supplier offers targeting the same canonical Product Master are considered.</p>
          </div>
          <div className="text-right">
            <span className="rounded-full bg-orange-50 px-3 py-1.5 text-xs font-bold text-orange-700">{demand.demand_id}</span>
            {scope.product_master_code && <p className="mt-2 text-xs font-mono text-slate-400">{scope.product_master_code}</p>}
          </div>
        </div>

        <div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Summary icon={<PackageCheck size={18} />} label="Quantity" value={`${demand.quantity ?? 0} ${scope.unit ?? "units"}`} />
          ...
          <Summary icon={<Scale size={18} />} label="Delivery" value={demand.geography ?? scope.qualification?.geography ?? "Not specified"} />
        </div>
      </header>

      {evaluated.length === 0 ? (
        <div className="mt-6"><EmptyState title="No compatible supplier offers" text="There are no offers linked to this canonical Product Master yet. An offer for another product will not appear here." /></div>
      ) : (
        <section className="mt-6 space-y-5">
          <div className="flex items-end justify-between px-1"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-orange-600">Compatible offers</p><h2 className="mt-1 text-xl font-black text-slate-950">Available offers</h2></div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">{evaluated.length} offer{evaluated.length > 1 ? "s" : ""}</span></div>
          {evaluated.map(({ offer, quantityOk, budgetOk, currencyOk, marketOk, matched }, index) => (
            <article key={offer.id} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-soft sm:p-8">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div><div className="flex items-center gap-3"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-orange-500 text-xs font-black text-white">#{index + 1}</span><h2 className="text-xl font-black text-slate-950">{offer.name}</h2></div><p className="mt-2 text-sm text-slate-500">Supplier location: {offer.geography ?? "Not specified"}</p></div>
